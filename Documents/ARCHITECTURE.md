...
- **Bot history** is attached to the selected Bot in the top Bot workspace. The selected Bot exposes **Previous runs · N**; expanding it renders the same newest-first immutable Run summaries inline below the Bot strip. Historical selection remains read-only. While inspecting history, the compact **Exit history** utility returns to the selected Bot's current state; visible info affordances explain this and the other Run Activity utilities.
